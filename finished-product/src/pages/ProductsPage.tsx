...
import { X, ChevronDown, Plus, Minus, Star, ShoppingCart } from 'lucide-react';
...
const ProductsPage: React.FC = () => {
  ...
  const availableColors = useMemo(() => {
    ...
    if (selectedCollections.includes('MALLORCA')) {
      ['Dolphin Gray', 'White'].forEach(c => colors.add(c));
    }
    if (selectedCollections.includes('SHADY')) {
      ['Gray Slim', 'Slim Oak', 'Slim White'].forEach(c => colors.add(c));
    }
  ...
  useEffect(() => {
    ...
    // Filter out colors that are no longer available based on new collections
    const colorsInUrl = searchParams.getAll('color');
    const validColors = colorsInUrl.filter(c => {
      if (collections.length === 0) return false;
      const possible = new Set<string>();
      if (collections.includes('MALLORCA')) ['Dolphin Gray', 'White'].forEach(pc => possible.add(pc));
      if (collections.includes('SHADY')) ['Gray Slim', 'Slim Oak', 'Slim White'].forEach(pc => possible.add(pc));
      return possible.has(c);
    });
  ...
  // Clear selected colors if they are no longer in availableColors
  useEffect(() => {
    if (selectedColors.length > 0) {
      const next = selectedColors.filter(c => availableColors.includes(c));
      if (next.length !== selectedColors.length) {
        setSelectedColors(next);
  ...
  }, [availableColors, selectedColors, searchParams, setSearchParams]);
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-start">
                  ...
                  <AnimatePresence mode="wait">
                    {availableColors.length > 0 && (
                      ...
                      >
                        ...
                        <div className="space-y-3">
                          {/* <!-- DYNAMIC: loop through colors here --> */}
          ...
          <div className="flex-grow">
            <div className={`grid gap-x-8 gap-y-16 ${showFilters ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-4'}`}>
              {/* <!-- DYNAMIC: loop through collection products here --> */}
